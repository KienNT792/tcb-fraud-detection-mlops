...
Required config: `SSH_DEPLOY_KEY` in Secrets, `GIT_AUTH_TOKEN` in Secrets if repo access needs auth, plus `GCP_DEPLOY_HOST`, `GCP_DEPLOY_USER`, and optional `DEPLOY_PATH` in either Secrets or Variables.
